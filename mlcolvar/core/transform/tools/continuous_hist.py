...
class LogHistogram(Transform):
    """
    Compute continuous logarithmic histogram using Gaussian kernels
    """
    ...
    def __init__(self,
                 ...
                 bins: int,
                 sigma_to_center: float = 1.0) -> torch.Tensor :
        """Computes the continuous logarithmic histogram of a quantity using Gaussian kernels
...
def test_log_histogram():
    x = torch.randn((5,100))
    x.requires_grad = True
    hist = LogHistogram(in_features=100, min=-1, max=1, bins=10, sigma_to_center=1)
    out = hist(x)
    out.sum().backward()
